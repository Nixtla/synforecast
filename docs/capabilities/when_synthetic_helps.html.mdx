...
generate more series, get a better model. Two benchmarks on this page
draw the real line: adding synthetic series to a model that already has
enough real data does not reliably lower error, but pretraining on
synthetic data and applying it zero-shot clearly helps a series with
little history. Beyond accuracy, synthetic data also earns its place
through coverage, robustness, and privacy.
...
>     beat the observed data alone — across panel sizes 3–30 and 20
>     seeds the win rate sat at a coin flip with no significant effect.
...
>     scratch by ~14% when each series kept only ~48 months of history,
>     and the edge reversed once history was ample.
>
> The pattern: synthetic data lowers error where the model is *starved*,
> not where it is already well-fed.
...
The augmentation result above is about a model that already has enough
real data. Pretraining tests the opposite corner: a model with *too
little* real history to learn from. We pretrain a small NHITS on a
synthetic corpus from [`pretraining_pool()`](balanced_pool), then
forecast M3 Monthly series while varying how much history each series
keeps — 48 months up to the full record. The corpus is generated
independently and never sees M3.
...
|-----|--------------|-------------------|----------------|------------------|---------------------|--------------------|-------------|
| 0   | 48 mo        | 1.632             | 1.309          | 1.624            | 13.7                | 69%                | 2.0e-163    |
| 1   | 72 mo        | 1.441             | 1.299          | 1.436            | 5.0                 | 58%                | 3.9e-44     |
| 2   | 108 mo       | 1.308             | 1.417          | 1.324            | -10.6               | 39%                | 2.8e-54     |
| 3   | full         | 1.238             | 1.409          | 1.262            | -14.6               | 34%                | 3.3e-107    |
...
about 14% against training from scratch — 69% of series improve, well
beyond noise — because the from-scratch model has too few windows to
learn seasonality the pretrained model already carries. The gap narrows
as history grows and reverses past roughly 100 months, where a
from-scratch model has enough data and wins outright.

Fine-tuning lands in between and rarely wins: on short histories it
drags the strong pretrained model toward the data-starved fit, and on
long ones it can’t beat from-scratch. The simple rule is zero-shot for
cold-start series, from-scratch once history is ample.

So synthetic pretraining pays off at the **cold-start** end — new or
short-history series — rather than as a blanket accuracy gain. That is
the regime foundation-model pretraining targets, here at small scale.
