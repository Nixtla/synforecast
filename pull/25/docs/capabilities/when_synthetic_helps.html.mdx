...
>     in every one of 10 seeds — and the edge reversed once history was
>     ample.
>
...
about 20% against training from scratch — it wins in all 10 seeds (p =
0.002, the smallest value a 10-seed paired test can produce) — because
the from-scratch model has too few windows to learn seasonality the
...
paired, multi-seed comparison on your own data, never on the final
holdout.
