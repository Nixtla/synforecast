...
`balanced_pool()` returns 42 pre-configured generator *instances* —
built from 15 of SynForecast’s 31 generator classes, one class per
behavioral niche — ARMA and exponential smoothing, long-range memory and
volatility clustering, intermittent demand, deterministic chaos, counts,
bounded and heavy-tailed processes, and more. It is the default corpus
behind [`generate_series`](../getting-started/quickstart), and a
...
domain (finance, say) dominates the pool. The list is ordered
round-robin across the niches, so any prefix spans as many distinct
behaviors as possible — a `generate_series` panel smaller than the pool
still gets one niche per series.
...
> Reach for the [`pretraining_pool()`](#pretraining-pool) preset when
> you want maximal breadth for foundation-model pretraining — it bundles
> those meta-generators (with `balanced_pool` by default).
