...
1. Map each uniform through the inverse CDF of its marginal:
   `x_i = F_i^{-1}(u_i)`.
...
<code>[IntoDataFrameT](#narwhals.stable.v2.typing.IntoDataFrameT)</code> | target_col\]; each unique_id is one correlated variable.
...
Type | Description
---- | -----------
<code>[ndarray](#numpy.ndarray)</code> | np.ndarray: Array of time series values (univariate).
